...
object JpvUtils {
  ...
  def linkExtractor(url:String): String ={
    ...
    val body4 = doc.body()
    var link = body4.getElementsByClass("s-btn s-btn__primary grid--cell ta-center  _apply _url js-apply js-url-apply d-block")
    if (link.isEmpty) {
      link = body4.getElementsByClass("s-btn s-btn__primary grid--cell ta-center  _apply js-apply d-block")
    }
    link.attr("href")
  }
}
